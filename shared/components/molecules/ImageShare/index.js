...
export default class ImageShare extends React.Component {
	...
	getUrl = done => {
		...
		api.shorten(url, { medium: 'social' }).then(done)
	}
	...
	handleMouse = () => {
		...
		this.setState(
			...
			() => {
				...
				var eventList2 = [].forEach.call(this.state.imgContainer, function(e) {
			...
			>
				...
				<SocialItemContainer>
					<CopyToClipboard
						SocialItem
						text={url}
						onCopy={this.onStoryCopied.bind(this)}
					>
						<SocialItem BGColor={'#c7c7c7'} BGDarkColor={'#949494'} order={'3'}>
							<SocialIcon className="fa fa-link" aria-hidden="true" />
						</SocialItem>
					</CopyToClipboard>
				</SocialItemContainer>
