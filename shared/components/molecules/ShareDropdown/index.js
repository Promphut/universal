...
class ShareDropdown extends React.Component {
	...
	onStoryCopied = val => {
		...
		if (val != null) api.incStoryInsight(val, 'share', 'share_dark', 1)
	}
	...
	render() {
		...
		buttons.push(
			<span>
				<i className="fa fa-facebook" aria-hidden="true" style={buttonStyle} />
				Share on Facebook
			</span>
		)
		buttons.push(
			<span>
				<i className="fa fa-twitter" aria-hidden="true" style={buttonStyle} />
				Share on Twitter
			</span>
		)
		buttons.push(
			<span>
				<i className="fa fa-linkedin" aria-hidden="true" style={buttonStyle} />
				Share on LinkedIn
			</span>
		)
		buttons.push(
			<span>
				<LineIcon
					injectStyle={LineIconPosController}
					width={'28px'}
					height={'28px'}
					color={theme.primaryColor}
					style={LineIconStyle}
				/>
				<span style={LineIconTextPosController}>Share on Line</span>
			</span>
		)
		buttons.push(
			<span>
				<FontIcon className="material-icons" style={buttonStyle}>link</FontIcon>
				Copy Link
			</span>
		)
		...
		const button = (
			<div onMouseLeave={this.onNotHover}>
				<div onMouseEnter={() => this.onHover(0)}>
					<FbShareButton
						url={config.FRONTURL + this.props.url}
						button={
							<FlatButton
								label={buttons[0]}
								labelStyle={{
            ...
            />*/}
				...
				<div onMouseEnter={() => this.onHover(4)}>
					<CopyToClipboard
						text={config.FRONTURL + this.props.url}
						onCopy={this.onStoryCopied}
					>
						<FlatButton
							label={buttons[4]}
							labelStyle={{
		...
		const buttonDropdown = (
			<FontIcon
				...
				style={{
					color: '#8f8f8f',
					fontSize: `${this.props.buttonSize}px`,
					paddingRight: utils.isMobile() ? '8px' : '0px'
				}}
			>
				keyboard_arrow_down
			</FontIcon>
		...
		return (
			<Dropdown
				...
				float="right"
				marginMobile="0px 0px 0px -160px"
