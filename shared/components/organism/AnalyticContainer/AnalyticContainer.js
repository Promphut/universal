...
export default class AnalyticContainer extends React.Component {
	...
	calculateRule() {
		...
		var rules = [
			...
			{
				status: this.titleFocusWordChecker(title, focusWord),
				text: RuleText[5]
			}
		]
		...
		if (!focusWord || (focusWord && focusWord.trim() == '')) {
			...
			rules.sort((a, b) => a.status - b.status)
			this.setState({
				ruleSet: rules
			})
		} else {
	...
	focusWordDensityChecker(content, focusWord) {
		...
		if (density > 3 || (density >= 0 && density < 1))
			return { result: 0, density }
		else if (
			(density >= 1 && density < 1.75) ||
			(density > 2.25 && density <= 3)
		) {
