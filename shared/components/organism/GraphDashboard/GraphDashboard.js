...
class Axis extends React.Component {
  ...
  render() {
    ...
    return <X className="axis" transform={this.props.axisType == 'x' ? translate : ''} />;
  }
}
...
class LineChart extends React.Component {
  ...
  render() {
    ...
    let xAxis = d3.axisBottom(x).tickSize(-h - 20).ticks(8),
      yAxis = d3.axisLeft(y).tickSize(-w - 20).ticks(6).tickFormat(tickVal => tickVal >= 1000 ? `${tickVal / 1000}K` : tickVal);
...
class GraphDashboard extends React.Component {
  ...
  getPublisherInsight = () => {
    ...
    api.getPublisherInsight(pid, action, null, null, from, to).then((ins) => {
      ...
      this.setState({ data, sum: ins.summary });
      // console.log(ins)
    });
  };
      ...
      <Wrapper width={width} style={{ ...style }}>
        ...
        <div className="row" style={{ marginTop: '50px' }}>
          ...
          <FlatButton
            ...
            icon={
              <FontIcon className="material-icons" style={{ color: '#c4c4c4', marginLeft: '15px' }}>
                keyboard_arrow_down
              </FontIcon>
            }
          >
