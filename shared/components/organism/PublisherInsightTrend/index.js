...
class PublisherInsightTrend extends React.Component {
  state = {
    data: {},
    open: false,
  ...
  getTrendInsight = (current, sort, subaction, filter, limit) => {
    api.getTrendInsight(this.props.insigth, subaction, filter, sort, limit, current).then((data) => {
      // console.log(data)
      this.setState({ data });
    });
  };
  ...
  renderTableRowColumn = (trend, view) => {
    ...
    return (
      <TableRowColumn style={{ ...styles.tableTextBody, color: trend ? color : '' }}>
        {trend
          ? <Trend>
            <FontIcon className="material-icons" style={{ ...styles.arrow, color }}>
                  ...
                  ((insigth == 'topstories' && isObject(entry.story)) ||
                    ...
                    <TableRow className="sans-font" key={index}>
                      ...
                      <TableRowColumn style={styles.tableTextBodyName}>
                        <Bold to={entry.story ? entry.story.url : '/'}>
                          {index + 1}. {entry.story.title}
                        </Bold>
                      ...
                      {insigth == 'topcolumns' &&
                      <TableRowColumn style={styles.tableTextBodyName}>
                        <Bold to={entry.column ? entry.column.url : '/'}>
                          {index + 1}. {entry.column.name}
                        </Bold>
                      </TableRowColumn>}

                      {insigth == 'topwriters' &&
                      <TableRowColumn style={styles.tableTextBodyName}>
                        <Bold to={entry.writer ? entry.writer.url : '/'}>
                          {index + 1}. {entry.writer.display}
                        </Bold>
                      </TableRowColumn>}

                      {this.renderTableRowColumn(entry.pastSevenDays, entry.view.pastSevenDays)}
                      {this.renderTableRowColumn(entry.aWeekAgo, entry.view.aWeekAgo)}
                      {this.renderTableRowColumn(entry.twoWeeksAgo, entry.view.twoWeeksAgo)}

                      <TableRowColumn style={styles.tableTextBody}>
                        <Trend>&nbsp;</Trend>
                        <Rate>
                          {entry.view.overall ? this.numberWithCommas(entry.view.overall) : '-'}
                        </Rate>
                      </TableRowColumn>
                    </TableRow>,
              )}
