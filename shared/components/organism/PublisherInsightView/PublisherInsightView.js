...
class PublisherInsightView extends React.Component {
  state = {
    data: {},
    open: false,
  ...
  getViewInsight = (current, sort, subaction, filter, limit) => {
    api.getViewInsight(this.props.insigth, subaction, filter, sort, limit, current).then((data) => {
      this.setState({ data });
    });
  };
                ...
                (entry, index) =>
                  ((insigth == 'topstories' && isObject(entry.story)) ||
                    ...
                    <TableRow className="sans-font" key={index}>
                      ...
                      {this.renderTableRowColumn(entry.pastSevenDays, entry.trend.pastSevenDays)}
                      {this.renderTableRowColumn(entry.aWeekAgo, entry.trend.aWeekAgo)}
                      {this.renderTableRowColumn(entry.twoWeeksAgo, entry.trend.twoWeeksAgo)}
