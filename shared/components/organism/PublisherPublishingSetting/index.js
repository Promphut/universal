...
class PublisherPublishingSetting extends React.Component {
  ...
  getAdmin = () => {
    api.getAdmins().then((admins) => {
      admins = admins.map(admin => ({ text: admin.username, value: admin.id }));
  ...
  render() {
    ...
    for (let i = 0; i < tags.length; i++) {
      tag.push(<MenuItem key={i} value={tags[i]._id} primaryText={tags[i].name} />);
    }
    // console.log('userToAdmin', userToAdmin, admins)
    const actions = [
      <FlatButton label="Cancel" primary onTouchTap={this.handleClose} />,
      <FlatButton label="Confirm" secondary onTouchTap={this.deleteAdmin} />,
    ];
    ...
    return (
      <Container>
        ...
        <Flex>
          ...
          <Edit>
            <div className="row">
              <TextField hintText="Search Tags ..." onChange={this.filterTags} ref="searchText" />
              <i
