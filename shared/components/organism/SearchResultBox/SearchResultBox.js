...
export default class SearchResultBox extends React.Component {
  render() {
    ...
    if (this.props.isLoading) {
      resultFeed = (
        <NotFoundTextContainer><NotFoundText>กำลังค้นหา</NotFoundText></NotFoundTextContainer>
      );
      ...
      for (var i = 0; i < this.props.result.length; i++) {
        resultFeed.push(
          <NewsBox
            final={i == this.props.result.length - 1}
            detail={this.props.result[i]}
            key={i}
          />,
        );
      }
      ...
      resultFeed = (
        <NotFoundTextContainer>
          <NotFoundText>ไม่พบผลลัพธ์จากคำที่ค้นหา</NotFoundText>
        </NotFoundTextContainer>
      );
    }
