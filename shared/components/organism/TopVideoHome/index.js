...
class TopVideoHome extends React.Component {
  ...
  render() {
    ...
    var settings = {
      ...
      prevArrow: (
        <button type="button">
          <i style={{ left: '-15px' }} className="material-icons arrow">keyboard_arrow_left</i>
        </button>
      ),
      nextArrow: (
        <button type="button">
          <i style={{ left: '-15px' }} className="material-icons arrow">keyboard_arrow_right</i>
        </button>
      ),
      dots: true,
      responsive: [
      ...
      vdo.push(
        <Test key={i}>
          <MiniBox
            hover={hover}
            large={large}
            src={'/tmp/story-list/1486687443533-Dakota.jpeg'}
            opacity={-1}
          >
            <Gra hover={hover}>
              <Text>
                {truncate(
                  `“เน็ตมือถือไม่พอใช้” ปัญหาใหญ่ของผู้ใช้สมาร์ท
    ...
    return (
      <Content className={className} style={{ padding: '50px 0 80px 0' }}>
        <Feed>
          ...
          <Dash style={{ margin: '5px 0 30px 0' }} />
