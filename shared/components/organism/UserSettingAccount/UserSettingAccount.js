...
class UserSettingAccount extends React.Component {
  ...
  fetechUser = (e) => {
    ...
    api.getUser(null, auth.getToken()).then((user) => {
  ...
  signFB = (e) => {
    ...
    Request.get(`${config.BACKURL}/auth/facebook?publisher=${config.PID}`)
      .set('Accept', 'application/json')
      .end((err, res) => {
        if (err) {
          this.setState({
            textStatus2: 'Connect with Facebook fail',
            error2: true,
          });
        } else {
          this.setState({
      ...
      <div>
        <Container onSubmit={this.updateData} className="marginTop">
          ...
          <div className="row hidden-des">
            <TextStatus
              style={{ color: error2 ? '#D8000C' : theme.accentColor, margin: '30px 0 0 15px' }}
          ...
          <div className="sans-font btn-row">
            ...
            <SecondaryButton
              label="Reset"
              onClick={this.fetechUser}
              style={{ float: 'left', margin: '0 20px 0 0' }}
            />
            <TextStatus
              style={{ color: error2 ? '#D8000C' : theme.accentColor }}
