...
const StoryTitle = ({ style, story, selectStatus }) => {
  ...
  return (
    <Cont style={{ ...style }}>
      ...
      <TitleLink to={url} className="sans-font">
        {selectStatus === STATUS.DRAFTED ? title : ptitle}
      </TitleLink>
    </Cont>
  );
};
...
class UserSettingStory extends React.Component {
  ...
  getStories = () => {
    ...
    api
      .getFeed('story', this.getCurrentFilter(), sort, null, currentPage, this.FEED_LIMIT, {
        allowUnlisted: true,
      })
      .then((result) => {
      ...
      <Container>
        ...
        <div className="row">
          <div className="col-12">
            <Section1 className="sans-font">
              <TabHead
                onClick={this.filterPublished}
                style={
                  selectStatus === STATUS.PUBLISHED ? { fontWeight: 'bold', color: '#222' } : {}
                }
              >
                {storiesCount[`${STATUS.PUBLISHED}`] || '0 '} Published
              </TabHead>
              <TabHead
                onClick={this.filterDrafted}
                style={selectStatus === STATUS.DRAFTED ? { fontWeight: 'bold', color: '#222' } : {}}
              >
                {storiesCount[`${STATUS.DRAFTED}`] || '0 '} Drafted
              </TabHead>
        ...
        >
          <MenuItem
            ...
            leftIcon={
              <FontIcon className="material-icons" style={{ color: theme.primaryColor }}>
                edit
              </FontIcon>
            }
          >
            ...
            : <Table>
              <TableHeader displaySelectAll={false} adjustForCheckbox={false}>
                <TableRow>
                  ...
                  <TableHeaderColumn style={{ width: '15%', textAlign: 'center' }}>
                      Column
                    </TableHeaderColumn>
                  <TableHeaderColumn style={{ width: '15%' }}>Stats</TableHeaderColumn>
                  <TableHeaderColumn style={{ width: '15%' }}>
                    {selectStatus === STATUS.PUBLISHED ? 'Published' : 'Drafted'}
                  </TableHeaderColumn>
              ...
              <TableBody showRowHover displayRowCheckbox={false}>
                {stories &&
                    stories.map((story, index) => (
                      <TableRow key={index}>
                        <TableRowColumn style={{ width: '40%', padding: '10px 0 10px 0' }}>
                          <StoryTitle story={story} selectStatus={selectStatus} />
                        </TableRowColumn>
                        <TableRowColumn
                          style={{
                            width: '10%',
                        ...
                        >
                          {story.writer &&
                            <Link to={story.writer.url}>{story.writer.display}</Link>}
                        </TableRowColumn>
                        <TableRowColumn
                          style={{
                            width: '15%',
                        ...
                        >
                          {story.column && <Link to={story.column.url}>{story.column.name}</Link>}
                        </TableRowColumn>
                        <TableRowColumn style={{ width: '15%' }}>
                          ...
                          Views
                          <br />
                          {story.shares ? story.shares.total : 0}
                          {' '}
                          Shares
                        </TableRowColumn>
                        <TableRowColumn
                          style={{ width: '15%', wordWrap: 'break-word', whiteSpace: 'pre-wrap' }}
                        >
                          {selectStatus === STATUS.PUBLISHED
                            ? moment(story.published).format('lll')
                            : moment(story.created).format('lll')}
                        </TableRowColumn>
                        <TableHeaderColumn
                          style={{
                            width: '5%',
