...
class ContactPage extends React.Component {
  ...
  sendMessage = () => {
    ...
    api
      .sendContactEmail(contactCat, contact)
      .then((contact) => {
        this.setState({
          saved: true,
        });
  ...
  componentDidMount() {
    api.getPublisherContactCats().then((contactCats) => {
    ...
    return (
      <ContactAndAboutContainer onLoading={this.props.onLoading}>
        <Wrapper>
          ...
          <Article className="content-font" style={{ marginTop: '25px' }}>
            {contactCat.desc}
          </Article>
          <textarea
            rows="10"
