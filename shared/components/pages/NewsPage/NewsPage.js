...
class NewsPage extends React.Component {
	...
	loadFeed = () => () => {
		...
		api
			.getFeed('news', { status: 1 }, 'latest', null, page, this.FEED_LIMIT)
			.then(result => {
	...
	loadFeed2 = () => () => {
		...
		// console.log('page', page)
		api
			.getFeed('news', { status: 1 }, 'trending', null, page, this.FEED_LIMIT)
			.then(result => {
						...
						>
							...
							<Latest>
								...
								>
									<div>
										{feedLatest.map((item, index) => (
											<NewsBox
												final={index == feedLatest.length - 1}
												detail={item}
												key={index}
												timeline
											/>
								...
								{!hasMoreFeed &&
									<SeemoreContainer>
										<SeeMore
											url={'/stories/all?type=news&sort=latest&page=1'}
										/>
									</SeemoreContainer>}
							</Latest>
							<Trending>
									...
									>
										<div>
											{feedTrend.map((item, index) => (
												<NewsBox
													final={index == feedTrend.length - 1}
													detail={item}
													key={index}
													timeline={false}
												/>
								...
								{!hasMoreFeed2 &&
									<SeemoreContainer>
										<SeeMore
											url={'/stories/all?type=news&sort=trending&page=1'}
										/>
									</SeemoreContainer>}
