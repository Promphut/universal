...
const SearchResultBoxAsync = asyncComponent({
  resolve: () =>
    import(/* webpackChunkName: 'SearchResultBox' */ '../../organism/SearchResultBox/index.js'),
});
...
export default class SearchResultPage extends React.Component {
  ...
  componentWillMount() {
    this.setState({
      keyword: utils.querystring('keyword', this.props.location)
        ? utils.querystring('keyword', this.props.location)
        : '',
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.match.params.type != this.props.match.params.type) {
      this.setState(
        ...
        () =>
          this.fetchResult(
            utils.querystring('keyword', nextProps.location),
  ...
  fetchResult = (keyword, type) => {
    if (!isEmpty(keyword)) {
      api
        .getStoryFromKeyword(keyword, type, this.state.currentPage, this.FEED_LIMIT)
        .then((result) => {
          this.setState({
  ...
  changeType = (type) => {
    if (type != this.state.type) {
      this.props.history.push({
        ...
        search: `?keyword=${this.state.keyword}&page=${1}`,
      });
    }
  };
          ...
          <Feed2>
            <FilterContainer>
              <FilterItem
                ...
                onClick={e => this.changeType('stories')}
                select={type === 'stories'}
              >
                STORIES
              </FilterItem>
              <FilterItem
                mobile={utils.isMobile()}
                onClick={e => this.changeType('news')}
                select={type === 'news'}
              >
                NEWS
              </FilterItem>
