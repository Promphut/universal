  ...
  render() {
    ...
    if (!isEmpty(story)) {
      if (isMobile) {
        likeBoxSize = 300;
        nextStoryContainer = (
          <NextStoryMobile cid={story.column._id} currentID={story._id} format={story.format} />
        );

        if (story.coverMobile.medium != `${config.BACKURL}/imgs/article_cover_portrait.png`) {
          hasCover = true;
        }
      } else {
        nextStoryContainer = (
        ...
        if (story.cover.medium != `${config.BACKURL}/imgs/article_cover_landscape.png`) {
          hasCover = true;
        }
      }
    }
