...
import {Footer, TopBarWithNavigation, SearchResultBox, Pagination, ArticleBox} from 'components'
...
export default class AllStoriesPage extends React.Component {
  ...
  reloadFeed = () => {
		...
		console.log(utils.querystring('page',this.props.location))
		console.log(utils.querystring('type',this.props.location))
		console.log(utils.querystring('sort',this.props.location))
  ...
  getAllFeed = () => {
	...
	changeSort = sort => {
		this.props.history.push({ hash: this.props.location.hash ,search: "?type=" + this.state.type  + "&sort="+ sort + "&page=" + this.state.currentPage + 1})
	...
	render() {
		let { isMobile, completed, totalPages, currentPage, loading, feed} = this.state
		return (
		<Wrapper>
			...
			<Content isMobile={isMobile}>
				...
				<Main>
					<TextLine className="sans-font hidden-mob">LATEST {toUpper(this.state.type)}</TextLine>
