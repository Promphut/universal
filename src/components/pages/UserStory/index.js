...
export default class UserStory extends React.Component {
	...
	loadFeed = () => {
	let  FEED_LIMIT = utils.isMobile() ? config.FEED_LIMIT_MOBILE*2 : config.FEED_LIMIT
	...
	let uid = this.state.user._id
		...
		api.getFeed('story', { writer: uid, status: 1 }, 'latest', null, currentPage, FEED_LIMIT)
			.then(result => {
				this.setState(
					{
						feed: result.feed,
						feedCount: result.feed.length!=0 ? (result.count['1'] ? result.count['1'] : 0) : 0,
						totalPages: result.feed.length!=0 ? utils.getTotalPages(FEED_LIMIT, result.count['1']) : 0,
					},
					() => {
...
showFullDescription = () => {
	...
	render () {
		...
		{
			let { user, isMobile, feedCount, feed, currentPage, totalPages, loading, showDescription } = this.state
			let FeedPack = "Loading"
					...
					<UserInfoSection role={auth.hasRoles(['ADMIN', 'EDITOR'])? 'admin' : 'user'}>
						{auth.hasRoles(['ADMIN', 'EDITOR']) &&
							<Card>
								...
								<SocialButtonContainer>
									<UserSocialBar ColorPack="dark" channels={user.channels}/>
								</SocialButtonContainer>
						...
						: <Main>
							...
							<Page>
								{!loading && totalPages > 0 && ((totalPages > currentPage && currentPage >= 0) ?
									...
									/>
									:
									<NoArticleHeader>ไม่มีข้อมูลในหน้านี้ กลับไปยัง<Link
												to={user.url+"?page=1"}
												style={{
